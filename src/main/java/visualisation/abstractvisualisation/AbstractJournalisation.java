...
public abstract class AbstractJournalisation {
  ...
  public interface Component extends AbstractJournalisation.Provides {
  }
  ...
  public static class ComponentImpl implements AbstractJournalisation.Component, AbstractJournalisation.Parts {
    private final AbstractJournalisation.Requires bridge;
    ...
    public ComponentImpl(final AbstractJournalisation implem, final AbstractJournalisation.Requires b, final boolean doInits) {
      this.bridge = b;
  ...
  private AbstractJournalisation.ComponentImpl selfComponent;
   ...
   *
   */
  protected AbstractJournalisation.Provides provides() {
   ...
   *
   */
  protected AbstractJournalisation.Requires requires() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   *
   */
  protected AbstractJournalisation.Parts parts() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   *
   */
  public synchronized AbstractJournalisation.Component _newComponent(final AbstractJournalisation.Requires b, final boolean start) {
    if (this.init) {
    ...
    this.init = true;
    AbstractJournalisation.ComponentImpl  _comp = new AbstractJournalisation.ComponentImpl(this, b, true);
    if (start) {
      _comp.start();
    }
    return _comp;
