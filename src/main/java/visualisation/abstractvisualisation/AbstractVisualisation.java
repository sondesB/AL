...
public abstract class AbstractVisualisation {
  ...
  public interface Component extends AbstractVisualisation.Provides  {
  }
  ...
  public static class ComponentImpl implements AbstractVisualisation.Component, AbstractVisualisation.Parts {
    private final AbstractVisualisation.Requires bridge;
    ...
    public ComponentImpl(final AbstractVisualisation implem, final AbstractVisualisation.Requires b, final boolean doInits) {
      this.bridge = b;
    ...
    private final class BridgeImpl_journalisation implements AbstractJournalisation.Requires {
      public final ITransfert envoyerMsgVersAffichage() {
        return AbstractVisualisation.ComponentImpl.this.affichage().recevoirMsgDeJournalisation();
      }
    }
  ...
  private AbstractVisualisation.ComponentImpl selfComponent;
   ...
   *
   */
  protected AbstractVisualisation.Provides provides() {
   ...
   *
   */
  protected AbstractVisualisation.Requires requires() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   *
   */
  protected AbstractVisualisation.Parts parts() {
    assert this.selfComponent != null: "This is a bug.";
    if (!this.init) {
   ...
   *
   */
  public synchronized AbstractVisualisation.Component _newComponent(final AbstractVisualisation.Requires b, final boolean start) {
    if (this.init) {
    ...
    this.init = true;
    AbstractVisualisation.ComponentImpl  _comp = new AbstractVisualisation.ComponentImpl(this, b, true);
    if (start) {
      _comp.start();
    }
    return _comp;
   ...
   *
   */
  public AbstractVisualisation.Component newComponent() {
    return this._newComponent(new AbstractVisualisation.Requires() {}, true);
  }
}
